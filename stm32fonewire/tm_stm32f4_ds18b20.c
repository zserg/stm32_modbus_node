...
uint8_t TM_DS18B20_Start(TM_OneWire_t* OneWire, uint8_t *ROM) {
	...
	if (!TM_DS18B20_Is(ROM) & !TM_DS18S20_Is(ROM)) {
		return 0;
	}
//        printf("DS18B20: _Start\n");	
	/* Reset line */
...
uint8_t TM_DS18B20_Read(TM_OneWire_t* OneWire, uint8_t *ROM, float *destination, uint16_t *raw_temp) {
	...
	/* Get sensor resolution */
        if(TM_DS18B20_Is(ROM)){          
     	   resolution = ((data[4] & 0x60) >> 5) + 9; // ds18b20
          ...
          switch (resolution) {
     	   ...
     	   resolution = 9; // ds18s20
	
           /* Store temperature integer digits and decimal digits */
           digit = (temperature >> 1) & 0x7F;
           decimal = (float)DS18B20_DECIMAL_STEPS_9BIT;
        }
